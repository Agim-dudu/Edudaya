...
def get_pretest_analysis(class_id: int) -> list[dict]:
    ...
    results = (
        db.session.query(
            ...
            PretestResult.created_at.label('submitted_at'),
        )
    ...
    return [
        {
            ...
            'score'       : row.score,
            'time_taken'  : _format_duration(row.time_taken),
            'has_done'    : row.score is not None,
        }
        for row in results
    ]

def _format_duration(seconds: int | None) -> str:
    """Mengonversi detik menjadi string 'MM:SS' untuk tampilan."""
    if seconds is None:
        return '-'
    minutes, secs = divmod(seconds, 60)
    return f'{minutes:02d}:{secs:02d}'

def get_student_ai_analysis_detail(teacher_id, user_id):
